...
class ChatRoom:
    def __init__(self, room_id, category, total_participants, participants, display_name, invite_code, allowed_type,
                 is_open, summary=None, created_by=None, created_at=None, active_media_key=None, udp_token=None, **kwargs):
        ...
        self.active_media_key = active_media_key
    ...
    def from_dict(cls, data: dict):
        ...
        return cls(
            ...
            active_media_key = data.get(Contract.ACTIVE_MEDIA_KEY),
            udp_token = data.get(Contract.UDP_TOKEN)
        )
...
class ChatScreen(BaseScreen):
    def __init__(self, parent, gui_state, chat_service):  # הוספנו את chat_service כדי לאפשר שליחה
        ...
        # אנחנו לא עושים לו grid() עדיין, הוא יופיע רק כשהשיחה תתחיל

        # רישום להאזנה למצב שיחה פעילה כדי להציג/להסתיר את הוידאו
        self.gui_state.register(StateKey.CALL_ESTABLISHED, self._toggle_video_panel)

    def _toggle_video_panel(self, call_data):
        if call_data:
            self.grid_columnconfigure(0, weight=1)
            self.video_container.grid(row=0, column=0, sticky="nsew", padx=(5, 0), pady=5)

            # יצירת חלון הוידאו ושמירתו במשתנה המפורש שלנו
            self.active_video_widget = VideoCallPanel(
                parent=self.video_container,
                room_id=call_data.get(Contract.ROOM_ID),
                media_key=call_data.get(Contract.MEDIA_KEY),
                udp_token=call_data.get(Contract.UDP_TOKEN),
...
class VideoCallPanel(ctk.CTkFrame):
    def __init__(self, parent, room_id, media_key, udp_token, gui_state, chat_service, **kwargs):
        ...
        # --- מבני נתונים לניהול דינמי של משתתפים ---
        self.remote_videos = {}  # מבנה: { sender_id: {"container": ctkFrame, "video_label": ctkLabel} }
        self.remote_last_seen = {}  # מבנה: { sender_id: timestamp } כדי לזהות ניתוקים מהרשת

        self.last_total_rows = 0
        self.last_total_cols = 0

        # --- הגדרת תצוגת ממשק המשתמש (UI) ---
        self.videos_frame = ctk.CTkFrame(self, fg_color="transparent")
        self.videos_frame.pack(fill="both", expand=True, pady=10)

        # יצירת המכולה (Container) למצלמה המקומית שלי - היא תמיד תוצג ראשונה ברשת החלונות
        self.my_video_container = ctk.CTkFrame(self.videos_frame, fg_color="#1E293B", corner_radius=10, border_width=1,
                                               border_color="#334155")
        self.my_video_label = ctk.CTkLabel(self.my_video_container, text="טוען מצלמה...")
        self.my_video_label.pack(fill="both", expand=True, padx=2, pady=2)

        my_name_label = ctk.CTkLabel(self.my_video_container, text="אני", font=("Heebo", 12, "bold"),
                                     text_color="#38BDF8")
        my_name_label.pack(side="bottom", fill="x", pady=2)

        # סידור ראשוני של חלון המצלמה המקומית
        self._rearrange_videos()

        self.disconnect_btn = ctk.CTkButton(
            self, text="📞 נתק שיחה", fg_color="#ef4444", hover_color="#dc2626", font=("Heebo", 14, "bold"),
            command=self._disconnect
        )
        self.disconnect_btn.pack(side="bottom", pady=20)

        self.cap = cv2.VideoCapture(0)
        self.remote_frame_queue = queue.Queue(maxsize=15)  # הגדלת התור כדי להכיל פריימים מרובי משתתפים

        # אתחול ה-MediaCommunicator עם ה-Public ID שלנו מהסטייט
        self.media_communicator = MediaCommunicator(
            server_ip="127.0.0.1",
            server_udp_port=8821,
            room_id=room_id,
            media_key=self.media_key,
            frame_queue=self.remote_frame_queue,
            udp_token=udp_token,
    ...
    def _rearrange_videos(self):
        # 1. ניקוי סידור קיים
        for widget in [self.my_video_container] + [item["container"] for item in self.remote_videos.values()]:
            widget.grid_forget()

        active_widgets = [self.my_video_container] + [item["container"] for item in self.remote_videos.values()]
        num_widgets = len(active_widgets)
        if num_widgets == 0: return

        max_cols = 2 if num_widgets <= 4 else 3

        # 2. איפוס משקלים לפני הגדרתם מחדש
        # ניקוי מהיר של כל העמודות והשורות
        for col in range(self.last_total_cols):
            self.videos_frame.grid_columnconfigure(col, weight=0)
        for row in range(self.last_total_rows):
            self.videos_frame.grid_rowconfigure(row, weight=0)

        # 3. סידור חדש
        for index, widget in enumerate(active_widgets):
            row = index // max_cols
            col = index % max_cols
            widget.grid(row=row, column=col, padx=6, pady=6, sticky="nsew")

        # 4. עדכון משקלים לחדשים
        total_rows = (num_widgets - 1) // max_cols + 1
        total_cols = min(num_widgets, max_cols)
        for r in range(total_rows):
            self.videos_frame.grid_rowconfigure(r, weight=1)
        for c in range(total_cols):
            self.videos_frame.grid_columnconfigure(c, weight=1)
    ...
    def _disconnect(self):
        ...
        self.chat_service.dispatcher.send_msg(MsgType.LEAVE_CALL, {Contract.ROOM_ID: self.room_id})

        # ניקוי פיזי של כל ווידג'טי הוידאו השמורים בזיכרון
        for item in self.remote_videos.values():
            item["container"].destroy()
        self.remote_videos.clear()

        self.gui_state.set_state(StateKey.ACTIVE_CALL_ROOM_ID, None)
        self.gui_state.set_state(StateKey.CALL_ESTABLISHED, None)
    ...
    def _update_local_video(self):
        ...
        if self.is_running:
            self.after(33, self._update_local_video)

    def _poll_remote_video(self):
        if not self.is_running:
            return

        # מילון מקומי זמני לטובת ריקון מהיר של התור ואיחוד פריימים (מניעת השהייה/Lag ברשת)
        latest_frames = {}
        try:
            while True:
                # קבלת הטאפל המעודכן מהפרוטוקול: (sender_id, frame_bytes)
                sender_id, frame_bytes = self.remote_frame_queue.get_nowait()
                latest_frames[sender_id] = frame_bytes
        except queue.Empty:
            pass
        ...
        # --- 1. עדכון ורינדור של פריימים חדשים שהתקבלו ---
        for sender_id, frame_bytes in latest_frames.items():
            self.remote_last_seen[sender_id] = now

            if sender_id not in self.remote_videos:
                container = ctk.CTkFrame(self.videos_frame, fg_color="#1E293B", corner_radius=10, border_width=1,
                                         border_color="#475569")

                video_label = ctk.CTkLabel(container, text="מתחבר...")
                video_label.pack(fill="both", expand=True, padx=2, pady=2)

                display_name = f"לא זמין"
                all_rooms = self.gui_state.get_state(StateKey.SYNC_ROOMS) or []

                # שימוש ב-next כדי למצוא את החדר (יחזיר None אם החדר לא נמצא)
                current_room = next((r for r in all_rooms if str(r.room_id) == str(self.room_id)), None)

                if current_room and current_room.participants:
                    display_name = current_room.participants.get(sender_id, display_name)

                name_label = ctk.CTkLabel(container, text= display_name, font=("Heebo", 12),
                                          text_color="#94A3B8")
                name_label.pack(side="bottom", fill="x", pady=2)

                self.remote_videos[sender_id] = {
                    "container": container,
                    "video_label": video_label
                }
                grid_changed = True

            # פענוח והצגת הוידאו בחלון המתאים
            try:
                ...
                if frame is not None:
                    ...
                    self.remote_videos[sender_id]["video_label"].configure(image=imgtk, text="")
            except Exception:
                pass


        stale_users = [s_id for s_id, last_time in self.remote_last_seen.items() if now - last_time > 4.0]

        for sender_id in stale_users:
            self.remote_videos[sender_id]["container"].destroy()
            del self.remote_videos[sender_id]
            del self.remote_last_seen[sender_id]
            grid_changed = True

        # אם התווסף משתמש חדש או הוסר משתמש ישן - נסדר מחדש את מטריצת ה-Grid
        if grid_changed:
            self._rearrange_videos()
...
class ChatHeader(ctk.CTkFrame):
    def __init__(self, parent, gui_state, chat_service, **kwargs):
        ...
        self.gui_state = gui_state
        self.chat_service = chat_service  # שמירת הרפרנס המקומי
    ...
    def _update_labels(self, room_obj):
        ...
        has_active_call = bool(room_obj.active_media_key)
    ...
    def _initiate_video_call(self):
        ...
        active_key = self.current_room_obj.active_media_key

        # הניתוב העסקי: אם יש מפתח - מצטרפים, אם אין - יוזמים
        if active_key:
            self.chat_service.join_video_call(room_id, active_key)
        else:
            self.chat_service.start_video_call_request(room_id)
