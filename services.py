...
class ChatService(BaseService):
    ...
    def _register_handlers(self):
        ...
        self.dispatcher.register(MsgType.JOIN_CALL, self.handle_start_call_response)
    ...
    def start_video_call_request(self, room_id):

        if self.gui_state.get_state(StateKey.ACTIVE_CALL_ROOM_ID) or self.pending_media_key:
            return
        self.gui_state.set_state(StateKey.ACTIVE_CALL_ROOM_ID, room_id)


        self.pending_media_key = Fernet.generate_key()

        payload = {
            Contract.ROOM_ID: room_id,
            Contract.MEDIA_KEY: self.pending_media_key.decode('utf-8')
        }
        self.dispatcher.send_msg(MsgType.START_CALL, payload)

    def join_video_call(self, room_id, active_media_key):
        if self.gui_state.get_state(StateKey.ACTIVE_CALL_ROOM_ID) or self.pending_media_key:
            return

        try:
            self.pending_media_key = active_media_key.encode('utf-8')
        except AttributeError:
            self.pending_media_key = active_media_key

        payload = {
            Contract.ROOM_ID: room_id
        }

        self.dispatcher.send_msg(MsgType.JOIN_CALL, payload)

    def handle_start_call_response(self, data, code):
        if code == MsgCodes.SUCCESS:
            self.gui_state.set_state(StateKey.ACTIVE_CALL_ROOM_ID, data.get(Contract.ROOM_ID))

            udp_token = data.get(Contract.UDP_TOKEN)
            self.gui_state.set_state(StateKey.CALL_ESTABLISHED, {
                Contract.ROOM_ID: data.get(Contract.ROOM_ID),
                Contract.MEDIA_KEY: self.pending_media_key,
