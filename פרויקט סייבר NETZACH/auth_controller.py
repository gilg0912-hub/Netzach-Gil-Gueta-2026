...
from app_constants import StateKey, Contract, AppScreens
...
class AuthController(ctk.CTkFrame):
    def __init__(self, parent, gui_state, services):
        super().__init__(parent, fg_color="#051224", width=310, height=430,
                         corner_radius=15, border_width=1, border_color="white")

        self.gui_state = gui_state
        self.auth_service = services.get(AppScreens.AUTH)
        self.timer_id= None
        self.text_label = ctk.CTkLabel(self, text='', text_color='red', font=('David', 14, 'bold'), fg_color='transparent', height=10)
    ...
    def change_screen_state(self, is_locked):
        ...
        if not screen:
            return
        if screen.confirm_btn is not None:
            if is_locked:
                screen.confirm_btn.configure(state='disabled')
                screen.lock= True
                self.state=False
            else:
                screen.lock= False
                screen.is_validate_fields()
                self.state=True
    ...
    def _update_countdown(self, remaining):
        ...
        if remaining > 0:
            self.text_label.configure(text= f'{ResponseTranslator.get_message(self.current_code)} {remaining}')
    ...
    def on_CODE_changed(self, code):
        ...
        if action_func:
            action_func()
        self.show_alert = policy.get('keep_alerts', False)
        self.text_label.configure(text= ResponseTranslator.get_message(code, **self.gui_state.get_state(StateKey.LAST_PAYLOAD) or {}), text_color=policy.get(UIKey.TEXT_COLOR, 'red'))
